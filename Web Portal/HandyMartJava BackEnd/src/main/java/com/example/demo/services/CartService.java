...
public class CartService {
	...
	public List<VendorProduct> searchProductsByName(String name,int customerId) {
		List<VendorProduct> searchedCartItem = vendorProductRepository.findByKeyword(name);
		return searchedCartItem;
	}
	
	
	public int getCartId(int customer_id) {
		   int cartid = cartRepository.findCartByUserId(customer_id).getCartId();
		   return cartid;
	}
	
	
	public void removeFromCart(int customerId, int subProductId) {
	    // Fetch the cart based on customer ID
	    Cart cart = cartRepository.findCartByUserId(customerId);

	    if (cart == null) {
	        throw new IllegalArgumentException("Cart not found for customer ID: " + customerId);
	    }

	    // Find the CartItem with the given SubProductCategory ID
	    CartItem cartItem = cart.getItems().stream()
	            .filter(item -> item.getCategories() != null && item.getCategories().getSubcategory_id() == subProductId)
	            .findFirst()
	            .orElseThrow(() -> new IllegalArgumentException("CartItem not found for SubProductCategory ID: " + subProductId));
